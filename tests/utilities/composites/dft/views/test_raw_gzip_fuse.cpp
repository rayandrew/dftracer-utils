...
std::string create_meta_trace(TestEnvironment& env) {
    ...
    dft_utils_test::compress_file_to_gzip_multimember(pfw, gz, 600);
    fs::remove(pfw);
    return gz;
}
...
bool rows_equal(const ResultTable& a, const ResultTable& b) {
    if (a.rows.size() != b.rows.size()) return false;
    for (std::size_t i = 0; i < a.rows.size(); ++i)
        if (a.rows[i].keys != b.rows[i].keys ||
            a.rows[i].values != b.rows[i].values)
            return false;
    return true;
}
...
TEST_SUITE("RawGzipFuse") {
    ...
    TEST_CASE("View.collect on a first-touch file bootstraps the index") {
        ...
        CHECK(boot.group_columns == ref.group_columns);
        CHECK(boot.value_columns == ref.value_columns);
        CHECK(boot.rows.size() == ref.rows.size());  // same groups, no metadata
    ...
    TEST_CASE("a POD-evaluable filtered first-touch aggregation bootstraps") {
        ...
        auto ref = View::from_file(gz, ref_idx)
                       ...
                       .collect()
                       .get();
        REQUIRE(ref.rows.size() == 1);  // only POSIX
        ...
        auto got = View::from_file(gz, fresh)
                       ...
                       .collect()
                       .get();
        CHECK(got.rows.size() == ref.rows.size());  // filter applied: 1 group
