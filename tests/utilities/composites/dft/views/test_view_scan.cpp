...
TEST_SUITE("View") {
    ...
    TEST_CASE("View - limit/offset paginate collect rows") {
        ...
        auto full = View::from_file(s.gz, s.idx)
                        ...
                        .collect()
                        .get();
        REQUIRE(full.rows.size() == 2);  // read (POSIX), fwrite (STDIO)
    ...
    TEST_CASE("View - group_by + agg (count/sum/min/max/mean)") {
        ...
        REQUIRE(table.group_columns.size() == 1);
        CHECK(table.group_columns[0] == "cat");
        REQUIRE(table.rows.size() == 2);  // POSIX, STDIO

        const ResultRow* posix = nullptr;
        for (const auto& r : table.rows)
            if (!r.keys.empty() && r.keys[0] == "posix") posix = &r;
        REQUIRE(posix != nullptr);
        // 30 POSIX events, dur = 10,11,...,39
        CHECK(posix->values[0] == doctest::Approx(30));         // count
        CHECK(posix->values[1] == doctest::Approx(30 * 24.5));  // sum(10..39)
        CHECK(posix->values[2] == doctest::Approx(10));         // min
        CHECK(posix->values[3] == doctest::Approx(39));         // max
        CHECK(posix->values[4] == doctest::Approx(24.5));       // mean
    }
    ...
    TEST_CASE(
        ...
        REQUIRE(table.group_columns.size() == 1);
        CHECK(table.group_columns[0] == "name");
        REQUIRE(table.value_columns.size() == 3);
        CHECK(table.value_columns[0] == "count");
        CHECK(table.value_columns[1] == "idle_pct");
        CHECK(table.value_columns[2] == "user_pct");

        REQUIRE(table.rows.size() == 1);
        const auto& r = table.rows[0];
        CHECK(r.keys[0] == "cpu");
        CHECK(r.values[0] == doctest::Approx(2));   // count
        CHECK(r.values[1] == doctest::Approx(40));  // idle_pct mean (60,20)
        CHECK(r.values[2] == doctest::Approx(60));  // user_pct mean (40,80)
    }
    ...
    TEST_CASE("View - from_directory scans a tree of traces") {
        ...
        for (const auto& row : table.rows) counts[row.keys[0]] = row.values[0];
