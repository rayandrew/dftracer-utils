...
// {group key -> value columns}, so two ResultTables compare regardless of row
// order.
std::map<std::string, std::vector<double>> rows_by_key(const ResultTable& t) {
    std::map<std::string, std::vector<double>> out;
    for (const auto& r : t.rows) {
        std::string k;
        for (const auto& part : r.keys) {
            k += part;
            k += '|';
        }
        out[k] = r.values;
