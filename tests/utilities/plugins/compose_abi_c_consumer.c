/* A real C consumer of the dftu_ext_compose ABI: this translation unit is
...
static const dftu_ext_compose* compose_of(const dftu_host* h) {
    return (const dftu_ext_compose*)h->get_extension(h->h, DFTU_EXT_COMPOSE);
}

/* (in*2) then (+10); returns the run task, writes *out and *rc. */
dftu_task* dftu_test_compose_pipe(const dftu_host* h, const int64_t* in,
                                  int64_t* out, int* rc) {
    const dftu_ext_compose* c = compose_of(h);
    if (!c) return NULL;
    dftu_op* a =
