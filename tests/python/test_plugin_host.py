#!/usr/bin/env python3
"""End-to-end tests for the compiled-plugin PluginHost binding.
...
class TestPluginHost:
    ...
    def test_stats_none_before_run(self):
        host = PluginHost()
        host.load(_PLUGIN)
    ...
    def test_emitted_blob_round_trips_to_python(self):
        with Environment() as env:
            ...
            for line in blob.decode().splitlines():
                ...
                total += int(count)
            assert total == 40
            assert host.stats["events_scanned"] == 40
