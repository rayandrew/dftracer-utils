...
struct ViewPlan {
    ...
    /// occ_bucket_us = occ_cell_us * 64. Smaller = tighter busy on short
    /// events, at more mask buckets per group (bounded by a per-window cap).
    /// Only honored when a time_range bounds the window; without one the grid
    /// falls back to a coarse safe default.
    ...
    /// (descending if `sort_desc`); `topk_col` then keeps the `topk_k` best
    /// rows (largest unless `!topk_largest`). Empty column = not applied.
    std::string sort_col;
    bool sort_desc = false;
    std::string topk_col;
    std::int64_t topk_k = -1;
...
#endif  // DFTRACER_UTILS_TRACE_VIEWS_VIEW_PLAN_H
