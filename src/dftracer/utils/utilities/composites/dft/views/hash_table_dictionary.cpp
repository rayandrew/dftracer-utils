...
#include <dftracer/utils/utilities/composites/dft/views/hash_table_dictionary.h>
#include <dftracer/utils/utilities/composites/dft/views/index_write_lock.h>
...
coro::CoroTask<bool> HashTableDictionary::commit(const CoverageSet& covered) {
    ...
    try {
        indexer::IndexDatabase ro(index_path_,
            ...
            if (it == existing.end())
                it = existing
                         .emplace(
                             type,
                             ro.query_hash_table(
                                 static_cast<indexer::IndexDatabase::HashType>(
                                     type)))
                         .first;
    ...
    try {
        indexer::IndexDatabase db(index_path_);
...
void HashTableDictionary::write_to_sink(indexer::IndexBatchSink& sink) const {
    for (const auto& [k, v] : sealed_) {
        const auto type = static_cast<std::uint8_t>(k[0] - '0');
