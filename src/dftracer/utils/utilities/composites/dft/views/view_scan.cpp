...
std::optional<common::query::Query> effective_query(const ViewPlan& plan) {
    RecordPhase rp;
    ...
    return common::query::parse_or_throw(q);
}
...
ViewScannerInput make_scanner_input(
    const ScanUnit& u, const ViewDefinition& vdef,
    const std::optional<common::query::Query>& q) {
        ...
        for (std::size_t fi = 0; fi < plan.files.size(); ++fi) {
            scope.spawn([&, fi](CoroScope&) -> coro::CoroTask<void> {
                ...
                if (uc_size == 0 || n_ckpts == 0) {
                    composites::dft::MetadataCollectorUtility meta;
                    auto md = co_await meta.process(
                        composites::dft::MetadataCollectorUtilityInput::
                            from_file(f.file_path)
                ...
                auto planned = co_await planner.process(pin);
...
coro::CoroTask<ExportStats> for_each_scanned_batch(
    ...
    // Before the whole-file units below join them in the same set.
