#ifndef DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_VIEW_AGGREGATE_H
#define DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_VIEW_AGGREGATE_H
...
// common/statistics/field_stat.h.
using common::statistics::FieldStat;
...
struct AggAccum {
    ...
    std::vector<common::statistics::DDSketch> sketches;
...
using GroupMap = std::unordered_map<std::string, AggAccum>;
...
// Column names and the final materialized table.
...
// Drop every column (and its aligned per-row cell) whose name is not in
// `select`, preserving result order. A no-op when `select` is empty or names a
// column that is not present. Apply only at the final collect() boundary, never
// to an intermediate table that will be merged further.
void project_columns(ResultTable& table,
                     const std::vector<std::string>& select);

}  // namespace dftracer::utils::utilities::composites::dft::views::detail

#endif  // DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_VIEW_AGGREGATE_H
