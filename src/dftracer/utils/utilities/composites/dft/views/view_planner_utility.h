#ifndef DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_VIEW_PLANNER_UTILITY_H
#define DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_VIEW_PLANNER_UTILITY_H
...
struct ViewPlannerInput {
    ...
    const std::vector<indexer::ChunkSpan>* cached_spans = nullptr;
    const std::vector<indexer::ChunkStatisticsResult>* cached_stats = nullptr;
    ...
    ViewPlannerInput& with_cached_chunks(
        const std::vector<indexer::ChunkSpan>* spans,
        const std::vector<indexer::ChunkStatisticsResult>* stats);
};
...
class ViewPlannerUtility
    : public Utility<ViewPlannerInput, Result<ViewPlannerOutput>> {
   public:
    coro::CoroTask<Result<ViewPlannerOutput>> process(
        const ViewPlannerInput& input) override;
};

}  // namespace dftracer::utils::utilities::composites::dft::views

#endif  // DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_VIEW_PLANNER_UTILITY_H
