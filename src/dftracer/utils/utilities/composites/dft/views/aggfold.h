#ifndef DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_AGGFOLD_H
#define DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_AGGFOLD_H
...
inline bool query_evaluable_by_fold(const common::query::Query& q) {
...
class AggFold : public Fold {
   public:
    ...
    void for_each_sorted_group(
        ...
        if (runs_.empty()) {
            std::vector<const std::pair<const std::string, AggAccum>*> ents;
    ...
    ResultTable result() {
        ...
        return to_result_table(m, *plan_);
    }
   ...
   private:
    ...
    GroupMap map_;
    std::string keybuf_;
    common::query::ValueMap qmap_;  // reused per-event predicate scratch
...
#endif  // DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_AGGFOLD_H
