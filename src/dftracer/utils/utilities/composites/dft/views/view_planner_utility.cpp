...
// ViewPlannerInput fluent builders
...
coro::CoroTask<Result<ViewPlannerOutput>> ViewPlannerUtility::process(
    ...
    if (input.view.query) {
        for (const auto& f :
             common::query::collect_fields(input.view.query->root())) {
            if (f != "ts") {
                query_prunable = true;
                break;
            }
    ...
    std::vector<indexer::ChunkSpan> chunk_spans_local;
    std::vector<indexer::ChunkStatisticsResult> chunk_stats_local;
    const std::vector<indexer::ChunkSpan>* chunk_spans = &chunk_spans_local;
    const std::vector<indexer::ChunkStatisticsResult>* chunk_stats = nullptr;
