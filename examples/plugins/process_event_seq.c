 ...
 * names each process emitted. Each event appends (e->ts, e->name) to the
 * host-owned mergeable map at key {pid}, whose value is a LIST_STR monoid. The
 * host merges the map across workers (list concat) and materializes it sorted
 * by ts to an Arrow table [k0 : int64 (pid), value : list<string> (the event
 * names in ts order)]. PluginHost::run returns it to Python as a pyarrow.Table
 * under "process_event_seq".
 *
 * Build: cc -std=c99 -shared -fPIC -I<repo>/include \
 *           -o process_event_seq.so process_event_seq.c
 */
...
static dftu_task* on_batch(void* slice, const dftu_batch* b,
    ...
    m = map->map_new(host->h, "process_event_seq", key_types, 1,
