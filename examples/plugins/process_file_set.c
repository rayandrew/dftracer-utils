/* Example dftracer-utils plugin in pure C: the SET of files each process
 * touched. Each file event adds e->fhash (an interned-string id) to the
 * host-owned mergeable map at key {pid}, whose value is a SET_STR monoid. The
 * host merges the map across workers (set union) and materializes it to an
 * Arrow table [k0 : int64 (pid), value : list<string> (the resolved file
 * labels)]. PluginHost::run returns it to Python as a pyarrow.Table under
 * "process_file_set".
 *
 * Build: cc -std=c99 -shared -fPIC -I<repo>/include \
 *           -o process_file_set.so process_file_set.c
 */
...
static dftu_task* on_batch(void* slice, const dftu_batch* b,
    ...
    uint32_t i;
    (void)slice;
    if (!map || !map->map_new || !map->map_add_u64) return NULL;
    m = map->map_new(host->h, "process_file_set", key_types, 1,
                     DFTU_MONOID_SET_STR);
    if (!m) return NULL;
    for (i = 0; i < b->count; ++i) {
        ...
        key[0] = (int64_t)e->pid;
        map->map_add_u64(host->h, m, key, (uint64_t)e->fhash);
    }
    return NULL;
}
