...
// written once and runs local, over MPI, over RPC, or anything else by swapping
// the implementation. Only the collectives a gather-then-merge reduction needs
// are exposed; add more as callers require. The interface itself has no MPI (or
// other transport) dependency, so it lives in core and everything can link it.
class Transport {
   public:
    ...
    // Gather each rank's `payload`; every rank receives all payloads ordered by
    // rank. The building block for distributed reductions (aggregate, etc.).
