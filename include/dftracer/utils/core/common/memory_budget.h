...
// `required_bytes` is the aggregated footprint (the measured result bytes, or a
// cheap estimate of the AGGREGATION-CF size), NOT the raw trace size.
...
// One-line warning for advice that does not fit (empty when it fits). Shared so
// the CLI binaries and the Python/dfanalyzer path surface the same message.
