#ifndef DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_MV_STORE_H
#define DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_MV_STORE_H
...
// Single-flight guard for one MV directory: an advisory, non-blocking exclusive
// lock on `<dir>/.lock`. Returns an fd to pass to unlock_view_dir on success,
// or -1 if another builder holds it (the caller should skip and let the winner
// finish). The lock is released on unlock and, on a crash, by the OS.
...
#endif  // DFTRACER_UTILS_UTILITIES_COMPOSITES_DFT_VIEWS_MV_STORE_H
