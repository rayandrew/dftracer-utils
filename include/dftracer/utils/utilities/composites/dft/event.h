...
struct DFTracerEvent {
    ...
    std::uint64_t ts = 0;
    std::uint64_t dur = 0;
    // Whether a "dur" field was present on the event. Duration statistics count
    // only events that carry a duration (mirrors the viz stats fold), so this
    // distinguishes a genuine dur:0 from an absent field.
    bool has_dur = false;
